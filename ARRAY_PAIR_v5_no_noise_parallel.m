...
function [iterTimes, PowerTotalItr, PowerFinalDistr, PhaseFinalDistr,... 
    ...
    ARRAY_PAIR_v5_no_noise_parallel(lambda_t, lambda_r, array_nt, array_nr, array_dt, array_dr, Gt, D, R_x, alpha)
...
%         disp(p_power);
        ...
        BS_R_power = BS_R_power .* alpha;
        if lambda_t >= 0.0093 % wavelength of 32GHz
            PA_Type = "HMC519";
        else
            PA_Type = "ADL8106";
        end
        BS_T_power = abs(Power_Amplify(BS_R_power, PA_Type));
        power_BS_emit(i) =(sum((BS_T_power)));  % 发射端PA放大后的功率
        
        BS_T_phase = BS_R_phase;
        ...
        %     filename = sprintf('./results/28_32/space_plot/d3r0itr%d.mat',i);
        ...
        MT_R_power = MT_R_power .* alpha;
        if lambda_r >= 0.0093 % wavelength of 32GHz
            PA_Type = "HMC519";
        else
            PA_Type = "ADL8106";
        end
        MT_T_power = abs(Power_Amplify(MT_R_power, PA_Type));
        % MT_T_power = MT_R_power;
        power_MT_emit(i) =(sum((MT_T_power)));     % 接收端返回给发射端的功率 

        MT_T_phase = MT_R_phase;
        ...
        if i > 1
            ...
            if (power_MT_recv(i) < power_MT_recv(i-1) * 0.9)
