...
function [iterTimes, Psum_BS_R, Psum_BS_T, Psum_MT_T, Psum_MT_R, PowerDistribFinal_BS_T, PowerDistribFinal_MT_T, PhaseDistribFinal_BS_T, PhaseDistribFinal_MT_T, ...
    Pos_p, Pos_r, Power_p_1, Phase_p_1, Ez_p, Ez_r] = ...
    ARRAY_PAIR_v4(lambda_t, lambda_r, array_nt, array_nr, array_dt, array_dr, Gt, D, R_x, gpuIdx)
...
% Both BS and MT has its respective Tx and Rx
    ...
    BS_T_phase = ones(1, array_nt*array_nt);
    MT_R_phase = ones(1, array_nt*array_nt);
    MT_T_phase = ones(1, array_nr*array_nr);
    BS_R_phase = ones(1, array_nr*array_nr);
    ...
    for iy = 0:array_nt-1
        for ix = 0:array_nt-1
            ...
            MT_R_pos(:, i_t) = [-dt_max+ix*array_dt; -dt_max+iy*array_dt + R_x; D];
            i_t = i_t + 1;
        end
    end
    % 接收阵元坐标
    for iy = 0:array_nr-1
        for ix = 0:array_nr-1
            MT_T_pos(:, i_r) = [-dr_max+ix*array_dr; -dr_max+iy*array_dr + R_x; D];
...
%         disp(p_power);
        ...
        BS_R_power = BS_R_power .* 0.025;
        BS_T_power = abs(Power_Amplify(BS_R_power, "HMC519"));
        power_BS_emit(i) =(sum((BS_T_power)));  % 发射端PA放大后的功率
        
        % PLL err = 0.5582
        Err = (rand(1,2500)-0.5).*0.5582; % Uniform error
        Err = randn(1, array_nt*array_nt).*0.5582; % normal distributed err
        % Err = 0;
        ...
        MT_R_power = MT_R_power .* 0.025;
        MT_T_power = abs(Power_Amplify(MT_R_power, "ADL8106"));
        power_MT_emit(i) =(sum((MT_T_power)));     % 接收端返回给发射端的功率 

        % PLL err = 0.5951
        Err = (rand(1,2500)-0.5).*0.5951;
        Err = randn(1, array_nt*array_nt).*0.5951;
        % Err = 0;
        ...
        % 保存第一次的发射端能量和相位分布
        if i == 1
            Power_p_1 = BS_T_power;
            Phase_p_1 = BS_T_phase;
        end
      
    end
    
    %%% 返回值
    Psum_BS_R = power_BS_recv;
    Psum_MT_T = power_MT_emit;
    Psum_BS_T = power_BS_emit;
    Psum_MT_R = power_MT_recv;

    PowerDistribFinal_BS_T = BS_T_power;
    PowerDistribFinal_MT_T = MT_T_power;
    PhaseDistribFinal_BS_T = BS_T_phase;
    PhaseDistribFinal_MT_T = MT_T_phase;
    Pos_p = BS_T_pos;
    Pos_r = MT_T_pos;
